...
public function update(){
	...
	if($this->name && $_SESSION["userid"]) {			
		...
		$checkAuthor = $this->conn->prepare("SELECT authorid FROM author WHERE name = ?");
		$checkAuthor->bind_param("s", $this->author);
		$checkAuthor->execute();
		$resultAuthor = $checkAuthor->get_result();

		if($resultAuthor->num_rows > 0) {
			$row = $resultAuthor->fetch_assoc();
			$authorid = $row['authorid'];
		} else {
			$insertAuthor = $this->conn->prepare("INSERT INTO author (name, status) VALUES (?, 'Enable')");
			$insertAuthor->bind_param("s", $this->author);
